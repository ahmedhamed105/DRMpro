...
import com.drm.model.entities.NavigationType;
...
public class MenuController extends AbstractManagedBean {
    ...
    public void init() {
        ...
        if (service != null) {
            ...
            if (session != null) {
                ...
                logger.debug(user);
                rootElements = service.getUserRootMenus();
    ...
    private List<Menu> buildUserMenuPermissions(User user) {
        ...
        final List<Menu> privilegesForUser = service.getAllMenus();//get all menues
    ...
    private void buildUserMenuTree(final List<Menu> privilegesForUser, List<Menu> rootElements) {
        ...
        CollectionUtils.forAllDo(rootElements, new Closure() {
            ...
            private UIMenu getUIMenu(Menu menu) {
                ...
                logger.debug("getUIMenu " + menu.getName());
...
//                uiMenu.setFunctionId(menu.getFunctionId());
            ...
            private List<UIMenu> getChildren(Menu menu) {
                logger.debug("getChildren of " + menu.getName());
                List<UIMenu> list = new ArrayList<UIMenu>();

                List<Menu> menuCollection = menu.getChildMenu();
                logger.debug("Children of " + menu.getName() + " are " + menuCollection);
                logger.debug("size of children " + menuCollection.size());
                logger.debug("loop on Children of " + menu.getName());
                logger.debug("all user menus " + privilegesForUser);
                for (Menu childMenu : menuCollection) {
                    logger.debug("child menu " + childMenu);
                    logger.debug("child name " + childMenu.getName());
                    if (privilegesForUser.contains(childMenu)) {// user
                        // permission
                        // has this
                        // menu
                        // child
                        logger.debug("all privilage menus for user contain this menu child " + childMenu.getName());
                        list.add(getUIMenu(childMenu));
                        logger.debug("child name " + childMenu.getName() + " added successfully to its parent " + menu.getName());
                    } else {
                        logger.debug("all privilage menus for user not contain this menu child " + childMenu.getName());
    ...
    public void selectItem(ActionEvent event) {
        ...
        Object object = event.getComponent().getAttributes()
                .get(MENU_CONTROL_ATTRIBUTE_MENU_URL);
        ...
        for (UIMenuLink uiMenuLink : uiMenuPermissions) {
            ...
            if (currentMenuURL != null && currentMenuURL.equals(menuURL)) {
                ...
                if (uiMenuLink.getNavigationType() == null
                        || uiMenuLink.getNavigationType().equals(
                                NavigationType.COMPOSITE)) {
                    inputForm.getChildren().clear();
                    includeCompositeComponent(
                            inputForm,
                            "components",
                            menuURL,
                            inputForm.getClientId().concat(
                                    "_".concat(String.valueOf(uiMenuLink
                                            .getId()))));
                    break;
                } else if (uiMenuLink.getNavigationType().equals(
    ...
    public void includeCompositeComponent(UIComponent parent, String libraryName, String resourceName, String id) {
        ...
        logger.debug("parent " + parent + " libraryName " + libraryName + " resourceName " + resourceName + " id " + id);
        // Prepare.
        ...
        UIComponent findComponent = faceletContext.getFacesContext()
